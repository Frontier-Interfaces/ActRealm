...
> **项目状态：** 功能实现已经推进到 M14，但当前源码仍是供本地测试的候选版本，
> 不是最终 v1 Release。M13 真实 Provider 验收和连续 48 小时 Runtime 稳定性门禁
> 仍未完成。
...
产品名称是 **ActRealm**。为保持兼容，当前 Runtime 可执行文件和 CLI 仍使用
`flow-agent` 这个名称。
...
Runtime 放入 `ActRealm.app/Contents/Helpers/flow-agent`。
...
| 范围 | 状态 | 能力边界 |
| --- | --- | --- |
| 本地 Runtime 与 Web 控制界面 | 当前测试候选版 | 功能实现到 M14；M13 真实 Provider 验收和 48 小时稳定性门禁仍待完成 |
| Claude Code 与 Codex | 当前构建 | 只覆盖本机会话；直接操作取决于事件是否带有真实回复通道 |
| 原生 macOS 客户端 | 可测试源码 | 已能在本地打包；台前调度与窗口激活仍需更广泛的人工验收 |
| Agent 工作区自动排布 | 实验性能力 | 需要 macOS 辅助功能权限，失败时不得修改 Runtime 任务状态 |
