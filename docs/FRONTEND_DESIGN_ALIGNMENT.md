...
1. `doctor` 的完整诊断报告和 Provider 兼容版本明细。
2. Claude 额度桥配置、Codex app-server Connector 手动接管和增强 Hook 模式。
3. 精确打开 Codex 对话、打开对应终端、只能打开应用、环境不支持四级跳转能力入口。
4. 会话恢复状态：可控制、仅观察、等待新事件、失去控制、已结束。
5. 简洁/详细/开发者三档字段选择器、开发者 ID 和独立详情抽屉。
6. Provider 静音、忽略、稍后提醒、交回原终端等高级处理入口。
7. 显式诊断采集、接入错误详情、兼容冲突和本地事件总数。
...
1. 每张任务卡的真实跳转能力按钮和失败反馈。
2. 恢复/控制能力标签，解释“能控制”和“只能观察”的差别。
3. Connector 和额度来源状态，以解决“已接入但没有数据”的排障问题。
4. 高级任务详情和字段选择器，但继续使用安全字段白名单。
5. 本机诊断页，用于定位 Runtime、Hook、Provider 版本和事件链路问题。
