...
- “进入 Outbox 的事件”使用独立开关控制四类事件是否进入 Outbox；关闭只影响呈现，不改变 Runtime 中的任务记录。
- 提示音开关。
- 自动保存由控件状态直接反馈，不重复弹出成功 Toast；读取或保存失败留在设置页内并提供重试。
- Runtime 重启和额度主动更新使用页面内持续状态；字段校验显示在对应输入项旁，不使用全局 Toast。
...
- Web 控制界面按设计不提供“智能聚焦（Agent Focus）”页面及其入口；macOS 原生客户端将其作为
...
合约，不另存一份设置或会话状态。除“智能聚焦（Agent Focus）”依然是 macOS
专属能力外，当前原生界面已接回：
