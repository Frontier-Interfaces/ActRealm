...
“Agent 任务”只展示仍在运行、仍在等待或带有当前可见待处理事项的会话。完成、
失败或空闲且没有当前请求的会话会立即离开主列表，但仍可按数据保留设置存在
本机历史中。运行超过 30 分钟不是隐藏条件。主标题优先来自 Claude/Codex 客户端
自己的会话标题；
...
可见位置；只要仍有待处理事项就不会被过滤掉。

权限请求分成两类，按钮不能混用：

1. **可在 ActRealm/Display 审批**：Hook/Connector 提供了本次请求的实时 reply
   channel 和 requestId。只有这类卡片支持下面的允许、拒绝和可选撤回；低、中、高
   风险都可以处理，风险等级只用于警示，不代替 Provider 权限或安全策略。
...
- **撤回**：原生 App 默认保留 3 秒撤回窗口。Display 可按 Provider 选择是否启用；
  关闭时立即写回且不可撤回；
- **去 Agent 处理**：只用于没有真实 reply channel 的观察型原生审批，或问题回答；
  可直接审批的权限请求不需要再交回 Agent 核实。
...
- `等待决定`：还没有选择结果；
- `3 秒内可撤回`：该 Provider 已启用撤回，决定尚未写回；
- `已发送`：撤回关闭时会立即进入该状态且不能再撤回；它只表示指令已经交给
  Provider，不能伪称 Provider 已执行；
...
当前原生版本每分钟刷新 Claude 额度，并在启动／唤醒后立即请求。设置中的“立即更新”
等待实际结果。凭据到期时会尝试由官方 Claude CLI 的 `/status` 会话续期；
`auth status` 仅用于检查未登录状态。无需发送模型对话，ActRealm 不另存或交换刷新令牌。
未登录、限流和网络失败会显示原因，旧值保持过期标记。真实长时间睡眠和凭据过期
恢复仍需单独验收。Fable 范围额度只要返回有效百分比就更新，不以 `is_active` 过滤。
...
- 统一数据导出和彻底清除；导出菜单再选择完整数据、聚合统计或 Token 数值格式。
- 额度卡显示来源、重置时间和最后采样；只改变显示密度，不改变额度数据与刷新规则。
- 任务卡显示任务、模型、状态、具体动作与时间；展开后可以查看计划、分页工作流、
  来源信息并回到原会话。只有实际收到的数据才会展示。
- 本机 Companion 配对与 Token 仪表板已启用。团队、Firebase 和手机／手表控制已移除；
  Review 与 Metadata Checkpoint 的原生入口仍暂停。
...
内置 `/status` 的有界后台会话，委托 Provider 维护凭据，再重新读取并验证变化。
...
Codex 额度读取是只读的实验能力。ActRealm 优先使用 Codex/ChatGPT 桌面应用内置
的较新 app-server 获取账户级标准额度、套餐和独立 Spark 窗口；全局 CLI 仍用于
Hook 与人工命令，但不会因为版本较旧而抢占 app-server。官方 Connector 短暂失败
时，尚未越过重置时间的最后一次官方额度会保留并标记为 stale，不会被只包含
Spark 的 rollout 覆盖。若从未成功确认标准套餐，则仍回退到有限大小的 rollout
尾部并严格校验 `rate_limits`，不会仅凭 Spark 数据猜测用户是 Pro。所有有效的
primary/secondary 周期都会保留；数据缺失、过期或格式不兼容时显示“不可用”。
...
- Token 来自本机 Claude transcript 或 Codex rollout 的结构化 usage。首次索引使用
  有界分段读取，完成后按增量刷新；扫描断点保存在本机私有 cache，Runtime 重启不再
  从零回放全部历史。
  卡片区分会话累计与最近调用／本轮数值；缓存不重复计入 Codex 输入，推理不重复
  计入输出。历史索引不完整时继续展示已观测数值和覆盖状态。仪表板提供组成、模型、
  项目／任务、热力图、趋势与数值导出；没有把部分数据宣称为完整账本。
...
- API 估算费用使用可识别模型及其带来源和日期的价格，支持本机价格缓存与后台价格刷新。
  它不是订阅账单或实际支出；模型未知、价格缺失时明确保留未定价部分，不用零元补位。
...
9. 设置、通知、额度不可用态和统计导出符合实际数据；依次切换额度卡“完整 / 紧凑 /
   单行”，确认三种结构与选项一致。拖动主窗口时，额度栏不小于 300 点，标题不被
   固定进度条挤压，进度条与三栏会随可用宽度伸缩；刷新 App 后选择仍保留。
