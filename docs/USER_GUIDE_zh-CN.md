...
- 本机使用统计、数据导出和彻底清除。
- 额度卡“完整 / 紧凑 / 单行”三种显示模式，默认使用完整模式。完整模式保留来源、
  重置与更新时间；紧凑模式使用标题/状态和用量/进度双行；单行模式把 Provider、
  额度周期、进度、百分比和状态压缩到一行。三种模式都会随窗口宽度自适应，额度栏
  最小宽度为 300 点，只改变显示密度，不改变额度数据、来源或刷新规则。
