...
> 当前已提交的 `agent/v1-full` 分支功能实现到 M14，实时用量、上下文、
> 估算 API 价和 Claude OAuth 额度已完成本机验收。M13 真实 Provider 最终复验和连续 48 小时发布门禁
...
ActRealm 只在收到匹配的 `PostToolUse`、Thread 状态清除或其他可靠终态时消退对应待
...
Runtime 会用桌面内置或 PATH 中的官方 Codex，在 `~/.actrealm/run/` 私有 Unix
...
并持久化该 Thread ID。ActRealm 重启后会重新 `thread/list` 并恢复这些显式托管
