...
数据源：`sessions[]`（WS 实时）。每个已接入 Provider 一行（无会话也显示，状态=空闲）。

### 12.1 行结构

`[Provider 图标] [名称 + 活动行(动效) + 进度条(条件)] [右侧状态标签]`

- 状态标签优先级：`等你`(橙, approval_owner=widget) > `终端等你`(橙描边, approval_owner=terminal) > `在跑`(蓝) > `空闲`(灰)；多件等待显示 `等你 ×N`；
- 点击行为：有等待 → Hero 切到该 Agent 的事项；在跑 → toast 显示完整活动；空闲 → 无操作。
...
| Provider | 采集 | 依据 |
|---|---|---|
| Claude | 安装可选的 statusline bridge（写入 `~/.flow-agent/bin/statusline`，缓存 rate_limits 到 `~/.flow-agent/cache/claude-rl.json`），读 5h/7d 窗口 | Open Island 验证；**不覆盖用户已有自定义 statusline，冲突时放弃并显示不可用** |
| Codex | **P1/实验性**：解析最近 rollout 中明确识别的 rate-limit 记录（只读、5 分钟轮询） | 内部格式不稳定；独立 adapter + fixture 版本门禁，未知 schema 立即不可用，不读取对话内容 |
| Gemini | 不采集 | 无可靠来源 |

### 13.1 展示

- 每 Provider 一组：`名称 · 窗口` + 百分比进度条（≥50% 绿 / 20–50% 橙 / <20% 红）+ `HH:MM 重置`；
...
- `~/.flow-agent/run` 目录 0700，Unix socket 0600；可用时校验 peer uid；消息大小上限 256KB；JSON 深度上限 32；
- **默认不持久化**：prompt 全文、完整命令、tool input/output、文件内容。活动 PermissionRequest 的完整命令只保存在 waiter 内存中，结束后立即释放；SQLite 只存脱敏 command_preview。events.summary 生成时执行脱敏；诊断模式才短期保留原始 payload，24h 自动清除；
