...
自动门禁的脱敏结果摘要：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 根因证据：`OutboxSection` 以 `outboxPageIndex` 选取 `entries[selectedIndex]`，点击时先写数字下标再调用可能改变排序的 `revealSession`。源码锚点见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
- 现场证据：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 证据：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。后端回答结构的自动化测试通过，说明缺口主要在原生 UI 投影。
...
- 根因证据：termination handler 只设置 `.failed`，没有 watchdog 或重启调度，见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
- 现场证据：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 证据：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 根因证据：重启清理只更新 attention 和 command，没有同步 sessions，源码见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
- 现场证据：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 证据：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)；源码 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 证据：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)、空闲对照 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 证据：真实最终状态 `response_finished / 子 Agent 已结束` 见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 证据：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。导出本身未包含 token，对照见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 证据：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。数据设置中“使用统计”容器也没有可读子项，截图见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 证据：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)、源码版本锚点 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 证据：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)、短测 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 证据：打包支持范围见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)；隔离首装证据见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 证据：源码中不存在睡眠/唤醒监听、WebSocket 无接收 deadline、额度刷新依赖 snapshot，见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。用户唤醒并恢复后的界面证据见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- 证据：[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)、事件聚合 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)、源码规则 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
- Rust/Swift 全自动化门禁通过，详见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
- Runtime 仅监听 `127.0.0.1`，不存在对局域网或公网的监听。
- `bridge.sock`、`codex-app-server.sock`、锁文件、数据库、WAL、缓存、备份均只允许当前用户访问，详见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
- 导出 JSON 有效，测试命令未明文出现，包含脱敏标记，没有 OAuth、Authorization 或 bootstrap token 字段，详见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
- Runtime 离线时阻塞 Hook 能静默成功返回，不会把 Provider 永久卡死，详见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。

### 4.2 Provider 行为

- Codex request-keyed 审批能得到正确 allow directive；匹配 PostToolUse 后移除 waiting 状态。
- Codex 原生 `request_permissions` 被识别为 Provider/终端拥有，只展示“打开应用”，没有伪造 ActRealm 批准按钮，证据 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
- auto-review 与 full-access 都不会创建竞争 waiter；任务完成后仍能产生完成确认，证据 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
- 最终 `doctor` 全项通过，真实 Claude/Codex 事件均有安装后记录，详见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。

### 4.3 首装与压力

- 隔离 HOME/ACTREALM_HOME 下，首屏如实显示未连接；Claude 安装/移除能写入和恢复测试配置；Codex 安装后要求用户在官方 `/hooks` 中信任，没有绕过官方步骤。
- 500 个唯一 Codex 会话、16 并发在 2.32 秒内写入；数据库为 500 session、500 event、0 open attention，`integrity_check=ok`；Runtime RSS 5888 KiB。有效结果见 [脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
- 第一次压力脚本因 shell 引号错误被明确判为无效，不计入产品结果；该结论保存在[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)。
...
原始日志、SQL 输出、源码锚点、界面截图、崩溃报告和压力脚本曾用于本次审计，但因可能包含本机路径、进程信息或用户内容，已从当前仓库树删除。仓库只保留[脱敏证据索引](evidence/2026-07-22/00-evidence-index.txt)，记录测试名称、结论和安全的聚合数字；没有保留真实导出 JSON。
